...
export class TrainingController {
  ...
  public async savingRelation() {
    ...
    // await this.teacherRepository.save([teacher1,teacher2]);
    const teacher1 = await this.teacherRepository.findOne({where: {id:9}});
    const teacher2 = await this.teacherRepository.findOne({where: {id:10}});
  ...
  public async removingRelation() {
    // const subject = await this.subjectRepository.findOne({where:{
    //   id:3},
    //    relations: ['teachers'],
    // });
